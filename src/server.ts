  ...
  constructor(base_dir: string, socket_emitter: Socket, user_config: { creators?: string }) {
    ...
    (async () => {
      ...
      this.commit_msg = await new Promise<string>((resolve, reject) => {
        ...
        child.execFile(gitPath, ['show', '--oneline', '-s'], opts, (error, stdout) => {
          if (error) {
            reject(error);
            return;
          }
    ...
    (() => {
      ...
      fs.watch(server_path, () => {
        ...
        for (const s in this.servers) if (current_servers.indexOf(s) < 0) untrack(s);
      });
    })();
    ...
    this.front_end.on('connection', (socket) => {
        ...
        switch (args.command) {
            ...
            async.series(
              [
                ...
                (cb) => {
                  ...
                  cb(!(whitelisted_creators.indexOf(username) >= 0) ? new Error() : null);
                },
                async.apply(instance.create, OWNER_CREDS),
                async.apply(instance.overlay_sp, args.properties),
              ],
              (err) => {
                if (!err) logging.info(`[${args.server_name}] Server created in filesystem.`);
                else {
          ...
          case 'create_unconventional_server':
            ...
            async.series(
              ...
              (err) => {
                if (!err) logging.info(`[${args.server_name}] Server (unconventional) created in filesystem.`);
          ...
          case 'download':
                    ...
                    (cb) => {
                      ...
                      try {
                        if (SOURCES[args.profile['group']].postdownload) {
                          SOURCES[args.profile['group']].postdownload!(profile_dir, dest_filepath)
                            .then(() => {
                              cb();
                            })
                            .catch((err) => {
                        ...
                        logging.error(`exception in postdownload server.js try/catch ${e}`);
                        cb();
                      }
                    },
                  ],
            ...
            break;
          case 'build_jar':
            let profile_path: string, working_dir: string, bt_path: string, dest_path: string, params: { cwd: string };
            try {
          ...
          case 'delete_build':
            ...
            if (args.type == 'spigot') {
              spigot_path = path.join(base_dir, DIRS['profiles'], 'spigot_' + args.version);
            } else {
          ...
          case 'copy_to_server':
            ...
            obj.execute((error, code) => {
              ...
              this.front_end.emit('host_notice', retval);
              for (const s in this.servers) this.front_end.emit('track_server', s);
            });
          ...
          case 'create_from_archive':
            ...
            let filepath: string;
            if (args.awd_dir) filepath = path.join(instance.env.base_dir, DIRS['archive'], args.awd_dir, args.filename);
            else filepath = path.join(instance.env.base_dir, DIRS['import'], args.filename);
      ...
      for (const server_name in this.servers) socket.emit('track_server', server_name);
    ...
    async.eachLimit(
      ...
      (server_name, callback) => {
        this.servers[server_name].onreboot_start((err) => {
          if (err) logging.error(`[${server_name}] Aborted server startup; condition not met:`, err);
          else logging.info(`[${server_name}] Server started. Waiting ${MS_TO_PAUSE} ms...`);
      ...
      const pool = new PromisePool<profile[], [string, collection]>(
        ...
        async ([name, profile]) => {
          try {
            ...
            logging.info(`Downloaded information for collection: ${name} (${output.length} entries)`);
            return output;
          } catch (e) {
            logging.error(
...
export class server_container {
  ...
  constructor(server_name, user_config, socket) {
    ...
    if ((user_config || {}).additional_logfiles) {
      ...
      additional = additional.map((e) => path.normalize(e).replace(/^(\.\.[/\\])+/, '')); //normalize path, remove traversal
    ...
    (() => {
      ...
      for (const i in default_skips) if (skip_dirs.indexOf(default_skips[i]) == -1) skip_dirs.push(default_skips[i]);
    ...
    const heartbeat = () => {
      ...
      this.intervals['heartbeat'] = setInterval(heartbeat, this.HEARTBEAT_INTERVAL_MS * 3);
      ...
      async.parallel(
        {
          ...
          query: (cb) => {
            this.instance.property('server.properties', (err, dict) => {
              if ((dict || {})['enable-query']) this.instance.property('query', cb);
        ...
        (err, retval) => {
          clearInterval(this.intervals['heartbeat']);
          this.intervals['heartbeat'] = setInterval(heartbeat, this.HEARTBEAT_INTERVAL_MS);
    ...
    this.intervals['world_commit'] = setInterval(world_committer, 1 * 60 * 1000);
        ...
        for (const i in required_args) {
          ...
          if (required_args[i] == 'callback')
            arg_array.push((err) => {
              ...
              if (err) logging.error(`[${server_name}] command "${args.command}" errored out:`, args);
            });
    ...
    this.nsp.on('connection', async (socket) => {
      ...
      const server_dispatcher = (args) => {
        ...
        try {
          ...
          while (this.notices.length > NOTICES_QUEUE_LENGTH) this.notices.shift();
        ...
        for (const i in required_args) {
          ...
          if (required_args[i] == 'callback')
            arg_array.push((err) => {
              ...
              args.time_resolved = Date.now();
              this.nsp.emit('server_fin', args);
              if (err) logging.error(`[${this.instance.server_name}] command "${args.command}" errored out:`, args);
        ...
        logging.info(`[${this.instance.server_name}] received request "${args.command}"`);
        fn.apply(this.instance, arg_array);
      };

      const produce_receipt = (args) => {
        /* when a command is received, immediately respond to client it has been received */
        logging.info(`[${this.instance.server_name}] ${ip_address} issued command : "${args.command}"`);
            ...
            async.waterfall(
              ...
              (err) => {
                if (err) {
                  ...
                  args.err = err;
                  args.time_resolved = Date.now();
                  logging.error(`[${this.instance.server_name}] command "${args.command}" errored out:`, args);
                  this.nsp.emit('server_fin', args);
                } else {
      ...
      const get_available_tails = () => {
        for (const t in this.tails) get_file_contents(this.tails[t].filename.replace(this.instance.env.cwd + '/', ''));
      };

      const get_prop = (requested) => {
        logging.info(`[${this.instance.server_name}] ${ip_address} requesting property: ${requested.property}`);
        this.instance.property(requested.property, (err, retval) => {
          logging.info(`[${this.instance.server_name}] returned to ${ip_address}: ${retval}`);
      ...
      const get_archives = () => {
        logging.debug(`[${this.instance.server_name}] ${username} requesting server archives`);
        this.instance.list_archives((err, results) => {
          if (err) logging.error(`[${this.instance.server_name}] Error with get_archives`, err, results);
          this.nsp.emit('archives', { payload: results });
        });
      };

      const get_increments = () => {
        logging.debug(`[${this.instance.server_name}] ${username} requesting server increments`);
        this.instance.list_increments((err, results) => {
          if (err) logging.error(`[${this.instance.server_name}] Error with get_increments`, err, results);
          this.nsp.emit('increments', { payload: results });
        });
      };

      const get_increment_sizes = () => {
        logging.debug(`[${this.instance.server_name}] ${username} requesting server increment sizes`);
        this.instance.list_increment_sizes((err, results) => {
          if (err) logging.error(`[${this.instance.server_name}] Error with get_increment_sizes`, err, results);
      ...
      const get_page_data = (page) => {
        switch (page) {
          case 'glance':
            logging.debug(`[${this.instance.server_name}] ${username} requesting server at a glance info`);
            ...
            async.parallel(
              {
                ...
                java_version_in_use: async.apply(this.instance.property, 'java_version_in_use'),
              },
              (err, results) => {
                if (err instanceof Object)
                  logging.error(`[${this.instance.server_name}] Error with get_page_data glance`, err, results);
      ...
      const manage_cron = (opts) => {
        ...
        switch (operation) {
          case 'create':
            ...
            logging.info(`[${this.instance.server_name}] ${ip_address} requests cron creation:`, cron_hash, opts);

            opts['enabled'] = false;

            async.series([async.apply(this.instance.add_cron, cron_hash, opts), async.apply(reload_cron)]);
            break;
          case 'delete':
            logging.info(`[${this.instance.server_name}] ${ip_address} requests cron deletion: ${opts.hash}`);

            try {
              this.cron[opts.hash].stop();
            } catch (e) {
              console.warn('Error deleting cron job: ', this.cron[opts.hash], e);
            }
            ...
            async.series([async.apply(this.instance.delete_cron, opts.hash), async.apply(reload_cron)]);
            break;
          case 'start':
            logging.info(`[${this.instance.server_name}] ${ip_address} starting cron: ${opts.hash}`);

            async.series([async.apply(this.instance.set_cron, opts.hash, true), async.apply(reload_cron)]);
            break;
          case 'suspend':
            logging.info(`[${this.instance.server_name}] ${ip_address} suspending cron: ${opts.hash}`);

            async.series([async.apply(this.instance.set_cron, opts.hash, false), async.apply(reload_cron)]);
            break;
          default:
            logging.warn(
      ...
      const connection_handler = (err) => {
        if (err) socket.disconnect();
          ...
          logging.info(`[${this.instance.server_name}] ${username} (${ip_address}) joined server namespace`);
  ...
  onreboot_start(callback) {
    async.waterfall(
      [
        ...
        (autostart, cb) => {
          logging.info(`[${this.instance.server_name}] autostart = ${autostart}`);
  ...
  broadcast_sp() {
    this.instance.sp((err, sp_data) => {
      logging.debug(`[${this.instance.server_name}] broadcasting server.properties`);
  ...
  broadcast_sc() {
    this.instance.sc((err, sc_data) => {
      logging.debug(`[${this.instance.server_name}] broadcasting server.config`);
  ...
  make_tail(rel_filepath) {
    ...
    */
    ...
    if (rel_filepath in this.tails) {
      logging.warn(`[${this.instance.server_name}] Tail already exists for ${rel_filepath}`);
      return;
    }

    try {
      const new_tail = new Tail(abs_filepath);
      logging.info(`[${this.instance.server_name}] Created tail on ${rel_filepath}`);
      ...
      logging.error(`[${this.instance.server_name}] Create tail on ${rel_filepath} failed: `, e);
      if ((e as any).errno != -2) {
        logging.error(e);
        return; //exit execution to perhaps curb a runaway process
      }
      logging.info(`[${this.instance.server_name}] Watching for file generation: ${rel_filepath}`);
      ...
      fw.on('add', (fp) => {
        if (abs_filepath == fp) {
          fw.clear();
          logging.info(`[${this.instance.server_name}] ${path.basename(fp)} created! Watchfile ${rel_filepath} closed`);
          async.nextTick(() => {

...
type incrementListItem = { step: string, time: string, size: string, cum: string };
...
export const dependencies: ReturnType<typeof async.memoize> = async.memoize((callback: async.AsyncResultCallback<{ [key: string]: any }>) => {
  async.parallel({
...
export const server_pids_up = () => {
  let cmdline, environ;
  const pids = fs.readdirSync(PROC_PATH).filter((e) => { if (/^([0-9]+)$/.test(e)) { return e } });
...
export default class mineos {
  ...
  read_ini = (filepath: string, callback: (arg0: NodeJS.ErrnoException | null, arg1?: { [key: string]: any }) => void) => {
  ...
  sp = (callback) => {
    ...
    async.waterfall([
      ...
      (stat_data, cb) => {
        if ( (fn in this.memoize_timestamps) &&
  ...
  create_from_archive = (owner, filepath, callback) => {
    ...
    if (filepath.match(/\//))  //if it has a '/', its hopefully an absolute path
      ...
      dest_filepath = path.join(this.env.base_dir, DIRS['import'], filepath);
      ...
      async.series<javaArgs>({
        ...
        'xms': (cb) => {
          this.sc((err, dict) => {
            ...
            cb(new Error(xmx >= xms && xms >= 0 ? undefined : 'XMS heapsize must be positive integer where XMX >= XMS >= 0'), xms);
          });
        },
        'jarfile': (cb) => {
          this.sc((err, dict) => {
        ...
        if (err) {
          ...
          if (results.xmx as number > 0)
            args.push(`-Xmx${results.xmx}M`);
          if (results.xms as number > 0)
  ...
  profile_delta = (profile, callback) => {
    ...
    async.waterfall([
      ...
      (incr_file_list, cb) => {
        ...
        cb(null, all_files.filter((n) =>{ return n.length }));
      }
    ], callback)
  }
        ...
        async.whilst(
          () => {
            if (iterations > MAX_ITERATIONS_TO_QUIT)
              ...
              return (this.server_name in server_pids_up())
          },
          (cc) => {
            iterations += 1;
  ...
  restart = (callback) => {
    async.series([
      async.apply(this.stop),
      async.apply(this.start)
    ], callback)
  }
    ...
    async.waterfall([
      ...
      (cb) => {
        cb(null, child_process.spawn(binary,
                                    ['-S', `mc-${this.server_name}`,
                                      '-p', '0', '-X', 'eval', `stuff "${msg}\x0a"`],
                                    params));
      }
    ], callback);
  }
    ...
    switch(property) {
      ...
      case 'server_files':
        ...
        async.waterfall([
          ...
          (sf, cb) => {
            server_files.push(...sf.filter((file) => {
          ...
          (sc_data, cb) => {
            ...
            fs.readdir(active_profile_dir, (err, files) => {
              if (err) {
                ...
                server_files.push(...files.filter((file) => {
                  return ((file.slice(-4).toLowerCase() == '.jar' && server_files.indexOf(file) < 0)
      ...
      case 'autosave':
        ...
        new_tail.on('line', (data) => {
          if (data.match(/INFO]: Saving is already turned on/)) { //previously on, return true
            clearTimeout(timeout);
            new_tail.unwatch();
            return callback(null, true);
          }
          if (data.match(/INFO]: Turned on world auto-saving/)) { //previously off, return false
            clearTimeout(timeout);
            new_tail.unwatch();

            this.stuff('save-off', () => { //reset initial state

...
import * as fireworm from 'fireworm';
      ...
      const webui_dispatcher = (args) => {
        ...
        switch (args.command) {
          ...
          case 'create_from_archive':
            ...
            let filepath: string;
            if (args.awd_dir)
              filepath = path.join(instance.env.base_dir, DIRS['archive'], args.awd_dir, args.filename);
            else
              filepath = path.join(instance.env.base_dir, DIRS['import'], args.filename);
            ...
            async.series([
              ...
              if (!err) {
                ...
                setTimeout(() => { this.front_end.emit('track_server', args.new_server_name) }, 1000);
              } else
                logging.error(err);
            });
            break;
        ...
        for (const i in listing) {
          ...
          if (match)
            spigot_profiles[match[0]] = {
              'directory': match[0],
              'jarfiles': fs.readdirSync(path.join(profiles_dir, match[0])).filter((a) => { return a.match(/.+\.jar/i) })
            }
        }
        cb();
      }
...
export class server_container {
  ...
  emit_eula() {
    async.waterfall([
      ...
      (accepted, cb) => {
        logging.info(`[${this.instance.server_name}] eula.txt detected: ${accepted ? 'ACCEPTED' : 'NOT YET ACCEPTED'} (eula=${accepted})`);
  ...
  try { fs.accessSync(this.instance.env.bwd, F_OK) } catch (e) { missing_dir = true }
  try { fs.accessSync(this.instance.env.awd, F_OK) } catch (e) { missing_dir = true }
  ...
  const heartbeat = () => {
    ...
    async.parallel({
      'up': (cb) => { this.instance.property('up',(err, is_up) => { cb(null, is_up) }) },
      'memory': (cb) => { this.instance.property('memory', (err, mem) => { cb(null, err ? {} : mem) }) },
      'ping': (cb) => {
        this.instance.property('unconventional', (err, is_unconventional) => {
          ...
          this.instance.property('ping', (err, ping) => { cb(null, err ? {} : ping) })
        })
      },
      'query': (cb) => {
  ...
  this.nsp.on('connection', (socket) => {
    ...
    const get_file_contents = (rel_filepath) => {
      if (rel_filepath in this.tails) { //this is the protection from malicious client
        ...
        async.waterfall([
          ...
          (data, cb) => {
            logging.info(`[${this.instance.server_name}] transmittting existing file contents: ${rel_filepath} (${data.length} bytes)`);
    ...
    const get_available_tails = () => {
      for (const t in this.tails)
        get_file_contents(this.tails[t].filename.replace(this.instance.env.cwd + '/', ''));
    }
    ...
    const get_prop = (requested) => {
      ...
      this.instance.property(requested.property, (err, retval) => {
        ...
        this.nsp.emit('server_fin', {'server_name': server_name, 'property': requested.property, 'payload': retval});
      })
    }
    ...
    const get_archives = () => {
      ...
      this.instance.list_archives((err, results) => {
        if (err)
          logging.error(`[${this.instance.server_name}] Error with get_archives`, err, results)
    ...
    const get_increments = () => {
      ...
      this.instance.list_increments((err, results) => {
        if (err)
          logging.error(`[${this.instance.server_name}] Error with get_increments`, err, results)
    ...
    const get_increment_sizes = () => {
      ...
      this.instance.list_increment_sizes((err, results) => {
        if (err)
          logging.error(`[${this.instance.server_name}] Error with get_increment_sizes`, err, results)
    ...
    const manage_cron = (opts) => {
          ...
          for (const cronhash in cron_dict) {
            if (cron_dict[cronhash].enabled) {
              try {
                ...
                logging.warn(`[${this.instance.server_name}] ${ip_address} invalid cron expression submitted:`, cron_dict[cronhash].source);
      ...
      switch (operation) {
        case 'create':
          ...
          logging.log(`[${this.instance.server_name}] ${ip_address} requests cron creation:`, cron_hash, opts);
          ...
          break;
        case 'delete':
          logging.log(`[${this.instance.server_name}] ${ip_address} requests cron deletion: ${opts.hash}`);
          ...
          break;
        case 'start':
          logging.log(`[${this.instance.server_name}] ${ip_address} starting cron: ${opts.hash}`);
          ...
          break;
        case 'suspend':
          logging.log(`[${this.instance.server_name}] ${ip_address} suspending cron: ${opts.hash}`);
          ...
          break;
        default:
          logging.warn(`[${this.instance.server_name}] ${ip_address} requested unexpected cron operation: ${operation}`, opts);
      }
    }
